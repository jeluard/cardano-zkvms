...
fn main() {
    ...
    if env::var_os("CARGO_FEATURE_SKIP_CYW43_FIRMWARE").is_none() {
        download_cyw43_firmware();
    }

    if env::var("CARGO_CFG_TARGET_ARCH").as_deref() == Ok("arm") {
        let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").expect("manifest dir"));
        let board_dir = manifest_dir.parent().expect("rp2350 board dir");
        println!("cargo:rustc-link-search={}", board_dir.display());
        println!("cargo:rustc-link-arg=-Tlink.x");
        println!("cargo:rustc-link-arg=-Trp2350-image.x");
        println!("cargo:rustc-link-arg=-Tdefmt.x");
        println!("cargo:rustc-link-arg=--nmagic");
        println!("cargo:rerun-if-changed={}", board_dir.join("memory.x").display());
        println!("cargo:rerun-if-changed={}", board_dir.join("rp2350-image.x").display());
    }
}
