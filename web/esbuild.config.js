...
// Contains agg_stark.vk — the aggregation STARK verifying key produced by
// `cargo openvm setup` (~/.openvm/agg_stark.vk). It is constant per OpenVM
// toolchain version and needed client-side to construct the full
// VmStarkVerifyingKey before STARK verification.
...
const patchOpenVMVerifierPlugin = {
  ...
  setup(build) {
    build.onStart(() => {
      ...
      const jsFile = path.join(verifierDir, 'openvm_wasm_stark_verifier.js');
...
import { __wbg_set_wasm, __wbindgen_init_externref_table } from "./openvm_wasm_stark_verifier_bg.js";
export * from "./openvm_wasm_stark_verifier_bg.js";
...
export default async function init(input) {
    ...
    const url = input ?? new URL("openvm_wasm_stark_verifier_bg.wasm", import.meta.url);
    const response = await fetch(url);
    const bytes = await response.arrayBuffer();

    const imports = {};
    imports["./openvm_wasm_stark_verifier_bg.js"] = await import("./openvm_wasm_stark_verifier_bg.js");
